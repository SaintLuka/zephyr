...
const Json &Json::operator[](const char* key) const { return (*m_ptr)[std::string(key)]; }
...
bool JsonValue::bool_value() const {
    throw std::runtime_error("Json::bool_value() error: Not boolean value");
}
...
/* JsonParser
 *
 * IndexedObject that tracks all state of an in-progress parse.
 */
struct JsonParser final {
    ...
    Json parse_json(int depth) {
        ...
        if (ch == '[') {
            ...
            while (1) {
                ...
                if (ch == ']')
                    break;
                if (ch != ',')
                    return fail("expected ',' in get_list, got " + esc(ch));
        ...
        return fail("expected get_cell_param, got " + esc(ch));
    }
};
}//namespace {
