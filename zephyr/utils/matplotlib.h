...
/// matplotlib in headless mode, for example on a machine with no display.
///
...
template<typename NumericX, typename NumericY, typename NumericZ, typename NumericColors>
bool scatter_colored(const std::vector<NumericX>& x,
             const std::vector<NumericY>& y,
             const std::vector<NumericZ>& z,
             const std::vector<NumericColors>& colors,
...
template<typename Numeric>
bool loglog(const std::vector<Numeric> &x, const std::vector<Numeric> &y, const std::map<std::string, std::string>& keywords)
{
    assert(x.size() == y.size());
    ...
    PyObject* res = PyObject_Call(detail::_interpreter::get().s_python_function_loglog, args, kwargs);
...
inline void figure_size(double w, double h, size_t dpi = 130)
{
    detail::_interpreter::get();
