...
int Cell::check_base_face_orientation() const {
    if (dim == 2) {
...
int Cell::check_base_vertices_order() const {
    if (dim == 2) {
        LargeList2D verts;
        for (int i = 0; i < 9; ++i) {
            verts[i] = vertices[i];
        ...
        auto cross_face = [](const Faces& faces, Side side1, Side side2) -> int {
        ...
        auto cross_face = [](const Faces& faces, Side side1, Side side2, Side side3) -> int {
            auto& face1 = faces[side1];
            auto& face2 = faces[side2];
...
int Cell::check_complex_faces() const {
    if (dim == 2) {
        for (int s = 0; s < 4; ++s) {
            auto f1 = faces[s];
            auto f2 = faces[s + 6];
