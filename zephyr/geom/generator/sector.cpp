...
Grid Sector::make() {
    ...
    if (!m_hole) {
        CV.resize(m_nx + 1, Array1D(m_nx + 1, nullptr));
        if (m_parts > 1) {
            CV2.resize(m_nx + 1, Array1D(m_nx + 1, nullptr));
            if (m_parts > 2) {
                CV3.resize(m_nx + 1, Array1D(m_nx + 1, nullptr));
                if (m_parts > 3) {
                    CV4.resize(m_nx + 1, Array1D(m_nx + 1, nullptr));
                }
            }
        }
    }
    ...
    Array2D SV(m_Nx + 1, Array1D(m_ny + m_Ny + 1, nullptr));
    ...
    if (!m_hole) {
        ...
        for (size_t i = 0; i <= m_nx; ++i) {
            ...
            for (size_t j = 0; j <= m_nx; ++j) {
                ...
                CV[i][j] = NodeInput::create(v0 + a * x + b * y + skew * x * y);
            }
        }
        ...
        for (size_t i = 0; i <= 2 * m_nx; ++i) {
            ...
            for (size_t j = 1; j < m_ny; ++j) {
                ...
                SV[i][j] = NodeInput::create(to_cartesian(r, phi));
            }
            SV[i][m_ny] = NodeInput::create(to_cartesian(m_r1, phi));
        }
        ...
        Array1D T(m_nx + 1, nullptr);
        Array1D R(m_nx + 1, nullptr);
        for (size_t i = 0; i <= m_nx; ++i) {
            R[i] = SV[i][1];
            T[i] = SV[2 * m_nx - i][1];
        }
        ...
        for (int k = 0; k < smooth_iter; ++k) {
            ...
            for (size_t j = 1; j < m_ny; ++j) {
                ...
                SV[0][j]->pos.x() = (b1.x() + b2.x() + 2.0 * b3.x()) / 4.0;
                SV[0][j]->pos.y() = 0.0;
                ...
                for (size_t i = 1; i < 2 * m_nx; ++i) {
                    ...
                    SV[i][j]->pos = (b1 + b2 + b3 + b4) / 4.0;
                }

                b1 = SV[2 * m_nx][j - 1]->pos;
                b2 = SV[2 * m_nx][j + 1]->pos;
                b3 = SV[2 * m_nx - 1][j]->pos;

                SV[2 * m_nx][j]->pos.x() = 0.0;
                SV[2 * m_nx][j]->pos.y() = (b1.y() + b2.y() + 2.0 * b3.y()) / 4.0;
            }

            // Сглаживание центральной части
            for (size_t j = 1; j <= m_nx; ++j) {
                b1 = CV[0][j - 1]->pos;
                b2 = j < m_nx ? CV[0][j + 1]->pos : T[0]->pos;
                b3 = CV[1][j]->pos;

                CV[0][j]->pos.x() = 0.0;
                CV[0][j]->pos.y() = (b1.y() + b2.y() + 2.0 * b3.y()) / 4.0;
            }

            for (size_t i = 1; i <= m_nx; ++i) {
                for (size_t j = 1; j <= m_nx; ++j) {
                    b1 = i < m_nx ? CV[i + 1][j]->pos : R[j]->pos;
                    b2 = CV[i - 1][j]->pos;
                    b3 = CV[i][j - 1]->pos;
                    b4 = j < m_nx ? CV[i][j + 1]->pos : T[i]->pos;

                    if (R[j] == T[i]) {
                        // особая точка
                        CV[i][j]->pos = (b1 + b2 + b3) / 3.0;
                    }
                    else {
                        CV[i][j]->pos = (b1 + b2 + b3 + b4) / 4.0;
                    }
                }
            }
            ...
            for (size_t i = 1; i <= m_nx; ++i) {
                b1 = CV[i - 1][0]->pos;
                b2 = i < m_nx ? CV[i + 1][0]->pos : R[0]->pos;
                b3 = CV[i][1]->pos;

                CV[i][0]->pos.x() = (b1.x() + b2.x() + 2.0 * b3.x()) / 4.0;
                CV[i][0]->pos.y() = 0.0;
            }
        }
        ...
        if (std::abs(m_part_angle - 0.5 * M_PI) > 1.0e-4) {
            ...
            auto transform = [this, xi](NodeInput::Ref v) {
                if (!v) { return; }
                ...
                v->pos.x() = r2 * std::cos(phi2);
                v->pos.y() = r2 * std::sin(phi2);
            };
        ...
        auto symmetrization = [this](NodeInput::Ref p1, NodeInput::Ref p2) {
            Vector3d v1 = p1->pos;
            Vector3d v2 = p2->pos;

            p1->pos.x() = 0.5 * (v1.x() + m_part_cos * v2.x() + m_part_sin * v2.y());
            p1->pos.y() = 0.5 * (v1.y() + m_part_sin * v2.x() - m_part_cos * v2.y());

            p2->pos.x() = 0.5 * (v2.x() + m_part_cos * v1.x() + m_part_sin * v1.y());
            p2->pos.y() = 0.5 * (v2.y() + m_part_sin * v1.x() - m_part_cos * v1.y());
        };
    ...
    if (!m_hole) {
        ...
        for (size_t r = 1; r < m_parts; ++r) {
            for (size_t j = 0; j <= m_ny; ++j) {
                for (size_t i = 1; i <= 2 * m_nx; ++i) {
                    SV[2 * m_nx * r + i][j] = NodeInput::create(rotate(SV[i][j]->pos, r));
                }
            }
        }
        ...
        for (size_t i = 0; i <= m_nx; ++i) {
            for (size_t j = 0; j <= m_nx; ++j) {
                ...
                if (m_parts > 1) {
                    CV2[m_nx - j][i] = NodeInput::create(rotate(v, 1));
                    if (m_parts > 2) {
                        CV3[m_nx - i][m_nx - j] = NodeInput::create(rotate(v, 2));
                        if (m_parts > 3) {
                            CV4[j][m_nx - i] = NodeInput::create(rotate(v, 3));
                        }
                    }
                }
            }
    ...
    if (!m_hole && std::abs(m_angle - 2 * M_PI) > 1.0e-4) {
        for (size_t i = 0; i <= m_nx; ++i) {
            CV[i][0]->bc = m_bounds.right;
        }
        if (m_parts == 1) {
            for (size_t i = 0; i <= m_nx; ++i) {
                CV[0][i]->bc = m_bounds.left;
            }
        } else if (m_parts == 2) {
            for (size_t i = 0; i <= m_nx; ++i) {
                CV2[i][0]->bc = m_bounds.left;
            }
        } else if (m_parts == 3) {
            for (size_t i = 0; i <= m_nx; ++i) {
                CV3[m_nx][i]->bc = m_bounds.left;
            }
        } else {
            for (size_t i = 0; i <= m_nx; ++i) {
                CV4[i][m_nx]->bc = m_bounds.left;
            }
        }
    }
    ...
    for (size_t i = 0; i <= m_Nx; ++i) {
        ...
        for (size_t j = 1; j < m_Ny; ++j) {
            ...
            SV[i][m_ny + j] = NodeInput::create(to_cartesian(r, phi));
        }
        SV[i][m_ny + m_Ny] = NodeInput::create(to_cartesian(m_r2, phi));
    }
    if (m_hole) {
        for (size_t i = 0; i <= m_Nx; ++i) {
            SV[i][0]->bc = m_bounds.inner;
        }
    }
    for (size_t i = 0; i <= m_Nx; ++i) {
        SV[i][m_ny + m_Ny]->bc = m_bounds.outer;
    }
    ...
    if (std::abs(m_angle - 2 * M_PI) < 1.0e-4) {
        ...
        for (size_t j = 0; j <= m_ny + m_Ny; ++j) {
            SV[0][j]->bc = m_bounds.right;
            SV[m_Nx][j]->bc = m_bounds.left;
        }
    }
    ...
    auto get_center_vface = [&](size_t x, size_t y, size_t r)
            -> std::array<NodeInput::Ptr, 2> {
        z_assert(x <= m_nx, "out of range");
        z_assert(y <= m_nx, "out of range");
        switch (r) {
            case 1: return {CV2[x][y], CV2[x][y + 1]};
            case 2: return {CV3[x][y], CV3[x][y + 1]};
            case 3: return {CV4[x][y], CV4[x][y + 1]};
            default: return {CV[x][y], CV[x][y + 1]};
        }
    };
