...
Grid Sector::make() const {
    using Array1D = std::vector<GNode::Ptr>;
    using Array2D = std::vector<std::vector<GNode::Ptr>>;
    ...
    if (!m_hole) {
        ...
        if (std::abs(m_part_angle - 0.5 * M_PI) > 1.0e-4) {
            ...
            auto transform = [this, xi](GNode::Ref v) {
                if (!v) { return; }
    ...
    if (!m_hole) {
        ...
        for (size_t r = 1; r < m_parts; ++r) {
            for (size_t j = 0; j <= m_ny; ++j) {
                for (size_t i = 1; i <= 2 * m_nx; ++i) {
                    SV[2 * m_nx * r + i][j] = GNode::create(rotate(SV[i][j]->pos, r));
                }
            }
        }
    ...
    std::vector<GNode::Ptr> vlist(4, nullptr);
