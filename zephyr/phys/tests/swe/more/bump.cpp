...
Bump::Bump(Parameters & par):Solution(par){
	...
	for (int i=0 ; i<=nx_ ; i++){
		xs_[i] = (i-0.5)*dx_;
		bed_[i] = MAX(0.0,0.2-0.05*pow(xs_[i]-10, 2.0));//One bump centered in x=10 m
	}
	...
	if(par.get_choice()==1){
		...
		depth_[2*nx_/5+1] = hmiddle; // temporary, for the Cardano method
...
void Bump::compute(){
	...
	if(solu==1){ 
		...
		for(int i=nx_-1; i>=0; i--){
			abcd(q_in, h_out, bed_[i],bed_[nx_],a,b,c,d);
			depth_[i] = height(p(a,b,c),q(a,b,c,d),a,b,depth_[i+1]);
		}

	}else if (solu==2){ 
		//transcritical without shock (subcritical-supercritical)

		for(int i=2*nx_/5; i>=0; i--){//Partie fluviale (before bump)
			abcd(q_in,hmiddle, bed_[i],zmax,a,b,c,d);
			depth_[i] = height(p(a,b,c),q(a,b,c,d),a,b,depth_[i+1]*(1+epsilon));
		}
		
		// As the bump is symmetric and the values do not coincide with the max of the topo, 
		// we make the solution deacrease. 
		
		abcd(q_in, hmiddle, bed_[2*nx_/5+1],zmax,a,b,c,d);
		depth_[2*nx_/5+1] = height(p(a,b,c),q(a,b,c,d),a,b,hmiddle);

		for(int i=2*nx_/5+2; i<nx_+1;i++){//Partie torrentielle (after bump)
			abcd(q_in, hmiddle, bed_[i],zmax,a,b,c,d);
			depth_[i] = height(p(a,b,c),q(a,b,c,d),a,b,depth_[i-1]*(1-epsilon));
		}
		...
		double test=100.0;
		double hminus=0.;
		int abslim = 2*nx_/5;							 // PB depending on NX_EX!!
		
		while(test>epsi && abslim < nx_){
			abcd(q_in,h_out,bed_[abslim],bed_[nx_],a,b,c,d);
			double hplus = height(p(a,b,c),q(a,b,c,d),a,b,h_out);
			abcd(q_in,hmiddle,bed_[abslim],zmax,a,b,c,d);
			hminus = height(p(a,b,c),q(a,b,c,d),a,b,hmiddle);
			test = RHJump(hplus, hminus, q_in);
			abslim++;
			}
			
		depth_[abslim] = hminus;
		
		/* Computation of the height */
		
		for(int i=abslim-1; i>=0; i--){// fluvial part (before bump)
			abcd(q_in,hmiddle,bed_[i],zmax,a,b,c,d);
			depth_[i] = height(p(a,b,c),q(a,b,c,d),a,b,depth_[i+1]+epsilon);
		}

		for(int i=nx_-1; i>abslim; i--){// torrential part (after bump)
			abcd(q_in,h_out,bed_[i],bed_[nx_],a,b,c,d);
			depth_[i] = height(p(a,b,c),q(a,b,c,d),a,b,depth_[i+1]);
		}
...
double Bump::p(double a, double b, double c) const{
...
double Bump::q(double a, double b, double c, double d) const{
...
double Bump::determinant(double p, double q) const{
...
double Bump::height(double p, double q, double a, double b, double hnear) const{
...
void Bump::abcd(double q_in, double h_out, double zbx, double zbfin, double &a, double &b, double &c, double &d){
...
double Bump::RHJump(double hplus, double hminus, double q) const{
...
void Bump::param(double L, double dx_ex) const{
