...
#include <zephyr/mesh/amr2/setup_geometry.h>
#include <zephyr/mesh/amr2/restore_connections.h>
#include <zephyr/mesh/amr2/remove_undefined.h>
#include <zephyr/mesh/amr2/sorting.h>
#include <zephyr/mesh/amr2/link_aliens.h>
...
{
    if (locals.empty()) {
        amr2::apply_impl<0>(locals, aliens, op, mesh);
    }
    else {
        auto dim = locals[0].dim;

        if (dim < 3) {
            amr2::apply_impl<2>(locals, aliens, op, mesh);
        } else {
            amr2::apply_impl<3>(locals, aliens, op, mesh);
        }
    }
}
#endif
