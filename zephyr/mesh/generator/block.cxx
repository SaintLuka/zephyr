...
Vertex::Ptr &Block::corner_vertex(int v_idx) {
    switch (v_idx) {
        case 0:
...
Vertex::Ptr &Block::boundary_vertex(int f_idx, int idx) {
    switch (f_idx) {
        case 0:
...
Vertex::Ptr &Block::preboundary_vertex(int f_idx, int idx) {
    switch (f_idx) {
        case 0:
...
void Block::create_vertices() {
    m_vertices.resize(m_size1 + 1, std::vector<Vertex::Ptr>(m_size2 + 1, nullptr));
    ...
    for (int i = 0; i <= m_size1; ++i) {
        ...
        m_vertices[i][0]       = Vertex::create(map2D(xi, -1.0));
        m_vertices[i][m_size2] = Vertex::create(map2D(xi, +1.0));
    }

    for (int j = 0; j <= m_size2; ++j) {
        double eta = (2.0* j - m_size2) / m_size2;

        m_vertices[0][j]       = Vertex::create(map2D(-1.0, eta));
        m_vertices[m_size1][j] = Vertex::create(map2D(+1.0, eta));
    }
    ...
    for (int i = 1; i < m_size1; ++i) {
        for (int j = 1; j < m_size2; ++j) {
            ...
            m_vertices[i][j] = Vertex::create(map2D(xi, eta));
        }
    }
...
void Block::link_vertices() {
    ...
    for (int v_idx = 0; v_idx < 4; ++v_idx) {
        ...
        std::vector<Vertex::Ptr> adj_vertices;
    ...
    for (int f_idx = 0; f_idx < 4; ++f_idx) {
        if (!boundary(f_idx) && !adjacent_block(f_idx)) {
            throw std::runtime_error("Face is not boundary and has no neigbor");
        }
...
void Block::add_vertices(VerticesList &vlist) {
    for (int i = 0; i < m_size1; ++i) {
        for (int j = 0; j < m_size2; ++j) {
            vlist.push_back(
                {
                    ...
                    m_vertices[i + 1][j]->v1,
                    m_vertices[i][j + 1]->v1,
                    m_vertices[i + 1][j + 1]->v1
                });
        }
    }
}

void Block::add_boundary_flags(BoundaryFlags &flist) {
    FaceFlag left, right, bottom, top;

    for (int i = 0; i < m_size1; ++i) {
        for (int j = 0; j < m_size2; ++j) {
            left   = FaceFlag::ORDINARY;
            right  = FaceFlag::ORDINARY;
            bottom = FaceFlag::ORDINARY;
            top    = FaceFlag::ORDINARY;

            if (i == 0 && m_boundaries[3]) {
                left = m_boundaries[3]->boundary_flag();
            }
            if (i == m_size1 - 1 && m_boundaries[1]) {
                right = m_boundaries[1]->boundary_flag();
            }
            if (j == 0 && m_boundaries[0]) {
                bottom = m_boundaries[0]->boundary_flag();
            }
            if (j == m_size2 - 1 && m_boundaries[2]) {
                top = m_boundaries[2]->boundary_flag();
            }

            flist.push_back({left, right, bottom, top});
        }
    }
}
