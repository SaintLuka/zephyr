...
generator::Base::Part::Part(int n_cells, int n_parts, int rank)
...
int generator::Base::Part::size() const {
    return to - from;
}
...
const std::string &Base::type() const {
    return m_type;
}
...
std::unique_ptr<Base> Base::create(YAML::Node config) {
    if (!config["type"]) {
...
void Base::check_params() const {

}

void Base::initialize(Storage &storage, Part part) {
    if (part.rank == 0) {
