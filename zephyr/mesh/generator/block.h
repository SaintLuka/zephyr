...
using BoundaryFlags = std::vector<std::array<FaceFlag, 4>>;
...
private:
    ...
    using Vertex_Ref = const std::shared_ptr<Vertex> &;
...
private:
    ...
    /// @brief Добавить ячейки блока в список вершин
    void add_vertices(VerticesList &vlist);

    /// @brief Добавить гран условия ячеек в список
    void add_boundary_flags(BoundaryFlags& flist);
