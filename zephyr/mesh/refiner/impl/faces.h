...
template <unsigned int dim, int side>
...
void setup_face_features(_face_& face, _vertices_& vertices);

template <>
void setup_face_features<2>(_face_& face, _vertices_& vertices) {
    using topology::iww;

    // Точка внутри ячейки
    Vector3d C = (Vector3d&)vertices.list[iww(1, 1)];

    // Вершины грани
    geom::ShortList1D vl = {
            (Vector3d &) vertices.list[face.vertices[0]],
            (Vector3d &) vertices.list[face.vertices[1]],
    };
...
template <>
void setup_face_features<3>(_face_& face, _vertices_& vertices) {
    using topology::iww;

    // Точка внутри ячейки
    Vector3d C = (Vector3d&)vertices.list[iww(1, 1, 1)];

    // Вершины подграней
    geom::ShortList2D vl = {
            (Vector3d &) vertices.list[face.vertices[0]],
            (Vector3d &) vertices.list[face.vertices[1]],
            (Vector3d &) vertices.list[face.vertices[2]],
            (Vector3d &) vertices.list[face.vertices[3]],
    };
...
/// @brief Устанавливает нормали и площади новых подграней
template <unsigned int dim>
void split_face_features(Faces& faces, _vertices_& vertices, int side) {
...
template <unsigned int dim, int side>
void split_face(Faces& faces, _vertices_& vertices) {
#if SCRUTINY
...
template <unsigned int dim>
void split_face(Faces& faces, _vertices_& vertices, int side);

template <>
void split_face<2>(Faces& faces, _vertices_& vertices, int side) {
    switch (side) {
        case Side::::LEFT:
...
template <>
void split_face<3>(Faces& faces, _vertices_& vertices, int side) {
    switch (side) {
        case Side::::LEFT:
...
/// Устанавливает площади и нормали новых граней.
template <unsigned int dim>
void split_face(Storage::iterator cell, int side) {
...
/// Устанавливает площади и нормали новых граней.
template <unsigned int dim>
void split_face(geom::Cell& cell, int side) {
    split_face<dim>(cell.faces, cell.vertices, side);
}

/// @brief Объединяет подграни в одну грань на стороне side
template<unsigned int dim, side side>
void merge_faces(Storage::iterator& cell) {
...
/// @brief Объединяет подграни в одну грань на стороне side
template<unsigned int dim>
void merge_faces(Storage::iterator& cell, side side);

template<>
void merge_faces<2>(Storage::iterator& cell, side side) {
    switch (side) {
        case Side::::LEFT:
...
template<>
void merge_faces<3>(Storage::iterator& cell, side side) {
    switch (side) {
        case Side::::LEFT:
