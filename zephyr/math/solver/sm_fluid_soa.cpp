...
void SmFluidSoA::update(Mesh &mesh) {
    // Определяем dt
    compute_dt(mesh);
...
void SmFluidSoA::compute_dt(Mesh &mesh) {
    double dt = mesh.min([this](Cell &cell) -> double {
        double c = m_eos->sound_speed_rP(curr[cell.index()].density, curr[cell.index()].pressure);
        return cell.incircle_diameter() / (curr[cell.index()].velocity.norm() + c);
    });
...
void SmFluidSoA::compute_grad(Mesh &mesh)  {
    compute_grad(mesh, get_current_sm);
}

void SmFluidSoA::compute_grad(Mesh &mesh, const std::function<smf::PState(Cell &)> &get_state)  {
    mesh.for_each([this, &get_state](Cell &cell) {
        ...
        d_dx[cell.index()] = grad.x;
        d_dy[cell.index()] = grad.y;
        d_dz[cell.index()] = grad.z;
    });
}

void SmFluidSoA::fluxes(Mesh &mesh) {
...
void SmFluidSoA::fluxes_stage1(Mesh &mesh)  {
    mesh.for_each([this](Cell &cell) {
        // Центр ячейки
        ...
        for (auto &face: cell.faces()) {
            ...
            auto face_extra = FaceExtra::Direct(
                    z_c, d_dx[i], d_dy[i], d_dz[i],
                    z_n, d_dx[j], d_dy[j], d_dz[j],
        ...
        half[cell.index()] = PState(q_c, *m_eos);
    });
}

void SmFluidSoA::fluxes_stage2(Mesh &mesh)  {
    mesh.for_each([this](Cell &cell) {
        // Центр ячейки
        auto& cell_c = cell.center();
...
void SmFluidSoA::swap(Mesh &mesh) {
    std::swap(curr, next);
}
