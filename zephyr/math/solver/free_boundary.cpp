...
void FreeBoundary::set_accuracy(int acc) {
    m_acc = std::min(std::max(1, acc), 2);  // 1 или 2
}

void FreeBoundary::set_axial(bool axial) {
    m_axial = axial;
}

void FreeBoundary::set_method(Fluxes method) {
    m_nf = NumFlux::create(method);
}

void FreeBoundary::set_limiter(const std::string& lim) {
...
void FreeBoundary::compute_grad(EuMesh &mesh) const {
...
void FreeBoundary::fluxes_stage1(EuMesh &mesh) const {
    mesh.for_each([this](EuCell &cell) {
        // Центр ячейки
...
void FreeBoundary::fluxes_stage2(EuMesh &mesh) const {
    mesh.for_each([this](EuCell &cell) {
        // Центр ячейки
        Vector3d cell_c = cell.center();
...
Distributor FreeBoundary::distributor(const std::string& type) const {
...
void FreeBoundary::set_flags(EuMesh &mesh) const {
    if (!mesh.adaptive()) {
        return;
    }
