...
namespace zephyr::math {
    ...
    /// @brief Интерполяционный шаблон на ячейках с индексами от b до e включительно.
    /// Пример: WENO<-1,1> — центральный шаблон на трех ячейках {-1,0,1}.
    ...
    public:
        ...
        /// @brief Интерполяция на правой грани ячейки (u(+1/2))
        double p() const;

        /// @brief Интерполяция на левой грани ячейки (u(-1/2))
        double m() const;
    ...
    /// @brief Класс WENO-реконструкции на пятиточечном шаблоне
    class WENO5 {
    public:
        WENO5(double u1, double u2, double u3, double u4, double u5);

        /// @brief Интерполяция на правой грани (u(1/2)(-))
        double p() const;

        /// @brief Интерполяция на левой грани (u(1/2)(+))
        double m() const;
