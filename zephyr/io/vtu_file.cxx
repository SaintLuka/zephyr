...
struct Handler {
    ...
    /// @brief Записать в файл координаты вершин элемента
    void write_points(std::ofstream &file, QCell& cell) {
    ...
    /// @brief Записать порядок вершин элемента
    void write_connectivity(std::ofstream &file, QCell& cell, index_t &counter) {
        index_t n = n_points(cell);
        for (index_t i = 0; i < n; ++i) {
            index_t val = counter++;
            file.write((byte_ptr) &val, sizeof(index_t));
...
void VtuFile::save(mesh::SoaMesh &mesh) {
    if (unique_nodes) {
