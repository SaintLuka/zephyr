...
public:
    ...
    virtual void initial(EuMesh& mesh, bool exact) const = 0;
...
public:
    ...
    void initial(EuMesh& mesh, bool exact) const final;
...
int main() {
    ...
    // Настройки метода
    solver.set_method(Solver::Method::VOF_CRP);
...
void BodyLine::initial(EuMesh& mesh, bool exact) const {
    for (auto cell: mesh) {
        if (!exact) {
...
void BodySquare::initial(EuMesh& mesh, bool exact) const {
    for (auto cell: mesh) {
        if (!exact) {
            cell(U).u1 = inside(cell.center());
        }
...
void BodyDisk::initial(EuMesh& mesh, bool exact) const {
    for (auto cell: mesh) {
